...
class Neo4j():
	...
	def connectDB(self):
		self.graph = Graph("http://10.89.23.225:7474", username="neo4j", password="123456")
...
# 		return answer
	...
	def matchItembyTitle(self,value):
		answer = self.graph.run('match (p:Item) where p.title="'+value+'" return p')
		return answer.data()

	# 根据title值返回互动百科item
	def matchHudongItembyTitle(self,value):
		answer = self.graph.run('match (p:HudongItem) where p.title="'+value+'" return p')
		return answer.data()

	# 根据entity的名称返回关系
	def getEntityRelationbyEntity(self,value):
		answer = self.graph.run("MATCH (entity1) - [rel] -> (entity2)  WHERE entity1.title = \"" +value +"\" RETURN rel,entity2").data()
		return answer
	...
	def findRelationByEntities(self,entity1,entity2):
		answer = self.graph.run("MATCH (n1:HudongItem {title:\"" + entity1 + "\"})- [rel] -> (n2:HudongItem{title:\""+entity2+"\"}) RETURN n1,rel,n2" ).data()
		if(len(answer) == 0):
			answer = self.graph.run("MATCH (n1:HudongItem {title:\"" + entity1 + "\"})- [rel] -> (n2:NewNode{title:\""+entity2+"\"}) RETURN n1,rel,n2" ).data()
		if(len(answer) == 0):
			answer = self.graph.run("MATCH (n1:NewNode {title:\"" + entity1 + "\"})- [rel] -> (n2:HudongItem{title:\""+entity2+"\"}) RETURN n1,rel,n2" ).data()
		if(len(answer) == 0):
			answer = self.graph.run("MATCH (n1:NewNode {title:\"" + entity1 + "\"})- [rel] -> (n2:NewNode{title:\""+entity2+"\"}) RETURN n1,rel,n2" ).data()
